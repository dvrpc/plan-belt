...
class SynchroTxt:
    ...
    def __init__(self, filepath: Path, simtraffic: Path = None) -> None:
        ...
        if simtraffic is not None:
            self.sim_dfs = SynchroSim(
                Path(simtraffic), "false"
            ).return_queues_for_intersection()
            ...
            pass
        self.excel_path = self.create_csv()
    ...
    def combine_synchro_sim(self, sim_df, synchro_df):
        ...
        for value in sim_df.index:
            if len(value[1]) == 1:
                synchro_df.at[(value), "simtraffic_queue"] = sim_df.at[(value)][
                    "95th Queue (ft)"
                ].tolist()
            elif len(value[1]) == 2:
                for char in value[1]:
                    breakout = (value[0], char)
                    synchro_df.at[breakout, "simtraffic_queue"] = sim_df.at[(value)][
                        "95th Queue (ft)"
                    ].tolist()
...
class SynchroSim:
    ...
    def __create_txt(self):
        ...
        """
        subprocess.run(
            f"/usr/bin/pdftotext -layout -colspacing 2 -nopgbrk '{self.filepath}'",
            shell=True,
            check=True,
        )
    ...
    def create_csv(self):
        ...
        return Path(self.dir, "sim_summary.xlsx")
